...
Future<String> AccionAlmSemiElab(Alm_semielab pAlm_semielab, String pstrHTTP) async {
  String respuesta = "";
  ...
  try{
    if (pstrHTTP != "") {
      if (pAlm_semielab.accion == "DELETE") {
        ...
        if (pAlm_semielab.idopt > 0 && pAlm_semielab.ccoste != "" &&
            pAlm_semielab.nave != "" && pAlm_semielab.pasillo != "") {
          Uri url = Uri.parse(pstrHTTP + strApiSemiElab);
          final http.Response response = await http.post(
            url,
            headers: <String, String>{
          ...
          if (response.statusCode == 201) {
            ...
            respuesta =
                "ERROR de servidor " + response.body.replaceAll("\"", "");
          }
        } else {
          respuesta = "ERROR: Debe informar todos los datos";
        }
